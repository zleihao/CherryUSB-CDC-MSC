...
#define USB_OTG_OUTEP(i)                                                                                               \
    ((DWC2_OUTEndpointTypeDef *)(USBD_BASE + USB_OTG_OUT_ENDPOINT_BASE + ((i) * USB_OTG_EP_REG_SIZE)))
...
int usb_dc_init(uint8_t busid)
{
    ...
    */
    ...
    USB_LOG_INFO("dwc2 has %d endpoints and dfifo depth(32-bit words) is %d, default config: %d "
                 "endpoints\r\n",
        endpoints,
        (USB_OTG_GLB->GHWCFG3 >> 16),
    ...
    USB_OTG_GLB->GINTMSK = USB_OTG_GINTMSK_USBRST | USB_OTG_GINTMSK_ENUMDNEM | USB_OTG_GINTMSK_OEPINT |
                           USB_OTG_GINTMSK_IEPINT | USB_OTG_GINTMSK_USBSUSPM | USB_OTG_GINTMSK_WUIM |
    ...
    /* xxx32 chips do not follow (USB_OTG_GLB->GHWCFG3 >> 16) if hsphy_type is zero, they
     * use 1.25KB(320 DWORD) */
    ...
    if (USB_EP_DIR_IS_OUT(ep->bEndpointAddress)) {
        ...
        if ((USB_OTG_INEP(ep_idx)->DIEPCTL & USB_OTG_DIEPCTL_USBAEP) == 0) {
            USB_OTG_INEP(ep_idx)->DIEPCTL |= (USB_GET_MAXPACKETSIZE(ep->wMaxPacketSize) & USB_OTG_DIEPCTL_MPSIZ) |
                                             ((uint32_t)USB_GET_ENDPOINT_TYPE(ep->bmAttributes) << 18) |
                                             (ep_idx << 22) | USB_OTG_DIEPCTL_SD0PID_SEVNFRM | USB_OTG_DIEPCTL_USBAEP;
        }
...
int usbd_ep_start_write(uint8_t busid, const uint8_t ep, const uint8_t *data, uint32_t data_len)
{
    ...
    /**
     * USB_OTG_DIEPTSIZ_PKTCNT 控制启动传输几次，每启动传输一次就是该端点的最大的包长
     * USB_OTG_DIEPTSIZ_XFRSIZ 是实际要发送的数据长度。
     * note: USB_OTG_DIEPTSIZ_PKTCNT = (实际发送的长度 + 该端点包最大值 - 1) / 该端点包最大值。
     *
     * 延伸：该算法为向上取整，示例：数据有4097，每次只能发送2048，计算需要发几次。
     * 常规版：n = 4097 / 2048; 4097 % 2048 == 0 ? n += 0 : n += 1;
     * 向上取整：n = (4097 + 2048 - 1) / 2048;
     */
    if (ep_idx == 0) {
...
void USBD_IRQHandler(uint8_t busid)
{
    ...
    if ((USB_OTG_GLB->GINTSTS & 0x1U) == USB_OTG_MODE_DEVICE) {
        ...
        /**
         * 如果没有使能USB DMA，那么只要主机发送请求，第一步是触发USB_OTG_GINTSTS_RXFLVL中断，
         * 该中断主要是用来读取主机发送的数据。
         */
        /* Handle RxQLevel Interrupt */
        if (gint_status & USB_OTG_GINTSTS_RXFLVL) { // RX FIFO 非空中断
...
#endif
        if (gint_status & USB_OTG_GINTSTS_OEPINT) { // OUT 端点中断
            ep_idx = 0;
            // 得到有那几个OUT端点中断，最多4个OUT端点
            ep_intr = dwc2_get_outeps_intstatus(busid);
            while (ep_intr != 0U) {
                if ((ep_intr & 0x1U) != 0U) { // 判断该端点是否触发中断
                    ...
                    if ((epint & USB_OTG_DOEPINT_XFRC) == USB_OTG_DOEPINT_XFRC) {
                        if (ep_idx == 0) {
                            if (g_dwc2_udc[busid].out_ep[ep_idx].xfer_len == 0) {
                                ...
                                usbd_event_ep_out_complete_handler(busid,
                                    0x00,
        ...
        if (gint_status & USB_OTG_GINTSTS_IEPINT) { // IN 端点中断
            ep_idx = 0U;
                    ...
                    if ((epint & USB_OTG_DIEPINT_XFRC) == USB_OTG_DIEPINT_XFRC) {
                        if (ep_idx == 0) {
                            ...
                            usbd_event_ep_in_complete_handler(
                                busid, 0x80, g_dwc2_udc[busid].in_ep[ep_idx].actual_xfer_len);

                            if (g_dwc2_udc[busid].setup.wLength && ((g_dwc2_udc[busid].setup.bmRequestType &
                            ...
                            g_dwc2_udc[busid].in_ep[ep_idx].xfer_len = 0;
                            usbd_event_ep_in_complete_handler(
                                busid, ep_idx | 0x80, g_dwc2_udc[busid].in_ep[ep_idx].actual_xfer_len);
                        }
                    }
        ...
        if (gint_status & USB_OTG_GINTSTS_USBRST) { // USB 复位中断
        ...
        if (gint_status & USB_OTG_GINTSTS_ENUMDNE) { // 枚举完成中断
        ...
        if (gint_status & USB_OTG_GINTSTS_PXFR_INCOMPISOOUT) { // 同步OUT传输未完成中断
            daintmask = USB_OTG_DEV->DAINTMSK;
            daintmask >>= 16;

            for (ep_idx = 1; ep_idx < CONFIG_USBDEV_EP_NUM; ep_idx++) {
                if ((BIT(ep_idx) & ~daintmask) ||
                    (g_dwc2_udc[busid].out_ep[ep_idx].ep_type != USB_ENDPOINT_TYPE_ISOCHRONOUS))
                    continue;
        ...
        if (gint_status & USB_OTG_GINTSTS_IISOIXFR) { // 同步IN传输未完成中断
            daintmask = USB_OTG_DEV->DAINTMSK;
            daintmask >>= 16;

            for (ep_idx = 1; ep_idx < CONFIG_USBDEV_EP_NUM; ep_idx++) {
                if (((BIT(ep_idx) & ~daintmask)) ||
                    (g_dwc2_udc[busid].in_ep[ep_idx].ep_type != USB_ENDPOINT_TYPE_ISOCHRONOUS))
                    continue;
        ...
        if (gint_status & USB_OTG_GINTSTS_SOF) { // 帧首中断
            USB_OTG_GLB->GINTSTS = USB_OTG_GINTSTS_SOF;
        }
        if (gint_status & USB_OTG_GINTSTS_USBSUSP) { // USB 挂起中断
            USB_OTG_GLB->GINTSTS = USB_OTG_GINTSTS_USBSUSP;
            usbd_event_suspend_handler(busid);
        }
        if (gint_status & USB_OTG_GINTSTS_WKUINT) { // 唤醒中断
