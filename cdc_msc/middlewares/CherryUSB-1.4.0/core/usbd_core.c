...
struct usbd_core_priv {
    ...
    bool zlp_flag;              //这个0长度数据包是当发送的数据包长度为端点的最大包的整数倍时发送的，用来告知主机数据发送完成。
...
void usbd_event_ep_in_complete_handler(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    if (g_usbd_core[busid].tx_msg[ep & 0x7f].cb) {
        g_usbd_core[busid].tx_msg[ep & 0x7f].cb(busid, ep, nbytes); //usbd_event_ep0_in_complete_handler
    }
}
...
void usbd_event_ep_out_complete_handler(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    if (g_usbd_core[busid].rx_msg[ep & 0x7f].cb) {
        g_usbd_core[busid].rx_msg[ep & 0x7f].cb(busid, ep, nbytes);  //usbd_event_ep0_out_complete_handler
    }
}
